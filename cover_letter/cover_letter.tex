...
We are pleased to submit our manuscript, ``From read-out geometry to in-silico
stimulation: a distributed functional-connectivity signature of Alzheimer's
disease,'' by Cristiano Capone, Enza Cece, Andrea Ciardiello, Guido Gigante,
Evaristo Cisbani and Maurizio Mattia, for consideration as an Article in
\textit{Nature Communications}.

Network-level neuromodulation is most effective when a single, well-defined site
can be engaged, yet Alzheimer's disease is widely regarded as a distributed
network disorder. Whether its connectivity signature can be reduced to a focal,
stimulable target has never been addressed causally, because doing so requires an
...
The central advance of our work is a clean conceptual distinction between two
ways of intervening on such a model: perturbing the connectivity \emph{graph}
(the fitted read-out kernel that defines the network's interactions) and
perturbing \emph{activity} (a focal oscillatory drive injected at a node). We
show these are fundamentally different interventions. Restoring control-like
