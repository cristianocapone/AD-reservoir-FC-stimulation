...
(AD), and the disorder is widely regarded as a distributed, network-level
process; yet whether its FC signature can be reduced to a few focal sites, or is
functionally irreducible to them, has not been tested causally, a question with
direct consequences for targeted neuromodulation.
...
-\mathbf{W}_{\mathrm{AD}}$ (Materials and Methods). Shifting an AD
...
one per unit amplitude (fig.~S8A,B). This frequency, $f_{\mathrm{eig}}\approx0.077$
...
(Fig.~\ref{fig:singlecompare}, figs.~S6,~S9); the two are therefore compared on a
...
$\bar{\mathbf{W}}_{\mathrm{CC}}$ (cf.\ fig.~S6). ($\alpha$ is distinct from the
