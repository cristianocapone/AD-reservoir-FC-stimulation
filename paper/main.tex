...
\usepackage{graphicx}
\graphicspath{{../paper_figures/}}     % self-contained preprint: figures bundled alongside
...
\begin{abstract}
\noindent
Targeted neuromodulation requires two decisions that are still made
heuristically: which site to engage, and at what amplitude. We address both with
individualised digital twins of brain dynamics. For each participant a
reservoir-computing model is fit to resting-state functional MRI so that its
free-running output reproduces that individual's own lagged connectivity, and we
show that the fitted models are identifiable across individuals. Because the
recurrent substrate is fixed and shared, and only a linear read-out is fit per
person, the twins live in common coordinates: they can be compared, averaged and
perturbed. Two interventions are then contrasted on the same models. Shifting a
twin's read-out toward a control template corrects the disease signature, but the
required correction is intrinsically distributed and no single-node edit
reproduces it. A focal oscillatory drive at the most-deviant region fails at any
amplitude, whereas a drive at the region selected by its \emph{effect on the
disease discriminant} succeeds from a single site. The two site sets are almost
disjoint: the most-deviant regions are the most weakly connected, and therefore
poor actuators, while effective sites are well-connected cortical nodes. Optimal
targets are individual, and a closed-loop controller that titrates amplitude
online from the model's own output reaches comparable efficacy at substantially
lower dose. Demonstrated here on Alzheimer's disease, the framework is a general,
model-based route to individualised target selection and dose minimisation.
\end{abstract}

% ============================================================================
\section*{Introduction}
Targeted and closed-loop neuromodulation is advancing quickly, but two decisions
at the centre of any such intervention are still made heuristically: which site
to engage, and at what amplitude. Targets are chosen anatomically, where
structural damage or network disruption is greatest, and doses are fixed in
advance rather than adapted to the individual receiving them. Both would be
better decided on a model of the particular brain being stimulated, one that can
be interrogated counterfactually before anything is delivered. That is the
promise of an individualised \emph{digital twin}: a generative model fit to one
person's own recorded activity, whose parameters can be edited and whose dynamics
can be driven, so that candidate interventions are evaluated in silico rather
than by trial.

Two properties decide whether such a twin is useful. It must be
\emph{identifiable} -- fit to an individual and demonstrably specific to them,
not a generic model with individual noise -- and it must be \emph{perturbable},
with a well-defined way to inject a drive at a chosen site and observe the
consequences for the whole system. Reservoir computing supplies both. A fixed
random recurrent network provides a shared dynamical substrate; only a linear
read-out is fit per individual, which makes per-subject estimation tractable from
short recordings and, because the substrate is common to everyone, places all
fitted models in the same coordinates, where they can be compared, averaged and
interpolated. The reservoir's separate input operator makes ``drive region $k$''
an explicit, well-posed operation rather than an ad hoc addition.

We demonstrate the framework on Alzheimer's disease, a setting where the
targeting question is live and unresolved. AD is increasingly understood as a
disorder of brain networks rather than of isolated regions, in which the
progressive disruption of large-scale functional interactions parallels cognitive
decline~\cite{seeley2009,delbeuck2003}.
...
use~\cite{buckner2013}. Although AD is generally
...
feature construction, reached AUROC $0.77\pm0.16$ (out-of-fold $0.76$;
...
i.e.\ the lagged correlations of the empirical parcel BOLD ($46$ CC, $34$ AD
...
(Fig.~\ref{fig:stim}A,B,D,E). In sharp contrast, applying the same correction to
...
minority of patients even at much larger interpolation strengths, again in both
read-outs (Fig.~\ref{fig:stim}C,F). Physiological
...
few steps, from a sliding window of the model's own past output (Methods). The
explicit window is a deliberately conservative choice: because the reservoir
satisfies the echo-state property, the relevant past is already embedded in its
instantaneous state, so an equivalent controller could read the biomarker
directly from $\mathbf{x}(t)$ without buffering past
output~\cite{albore2025search}.
...
$0.69\!\to\!0.66$; Fig.~\ref{fig:singlecompare}D--F). Online amplitude titration
...
misclassification of AD as CC from $21\%$ to $39\%$, so that only $20$ of the $33$ AD patients lie on the AD side of the boundary without stimulation, consistent
with the modest classifier accuracy; we therefore quantify the effect as the net
fraction of these AD-side patients that stimulation moves across to the CC side. A single \emph{population-average} resonant target, selected on the training patients alone, achieves a leakage-free net cure of $\sim$$25\%$, whereas each patient's \emph{personalised} LDA-resonant target moves $\sim$$90\%$ of them across ($100\%$ at $A=10$; Methods). The single-site reclassification is therefore a genuine, model-prescribed effect rather than an artefact of classifier leakage, and the gap between the two targets is itself the signature of target heterogeneity: no one site serves the group, which is precisely why the population-average target recovers only a minority. The personalised figure is nonetheless an upper bound, since each patient's site is chosen by argmax under the same discriminant that then scores it; the efficacy of a deployable protocol lies between the two arms and is not settled here.
...
the symptom circuit. The per-patient heterogeneity of the optimal sites ($25$ distinct parcels for $33$ patients, Fig.~S4) further argues that no universal
...
tractable prescription of optimal stimulation parameters. Selecting the target by its \emph{effect on the discriminant under a resonant drive} (the LDA-resonant criterion) jointly resolves drivability and direction, and is why it outperforms modal-coupling or disease-deviation selection.
...
patient can also \emph{close the loop} in real time. Titrating the drive's amplitude online against a sliding-window FC-lag estimate, using only the model's own past output rather than an offline oracle, reclassifies $\sim$$79\%$ of patients (approaching the $\sim$$88\%$ open-loop rate) while cutting both the stimulation dose ($\sim$$31\%$) and the departure from their own dynamics ($\sim$$5\%$ among patients reclassified under both protocols) relative to a fixed open-loop drive (Fig.~\ref{fig:singlecompare}). This is the in-silico counterpart of adaptive (closed-loop) neuromodulation, which seeks a minimal, causally available perturbation that crosses the diagnostic boundary rather than a supra-physiological open-loop drive. We emphasise, however, that these are \emph{model-level} prescriptions:
...
\paragraph{Why a reservoir rather than an autoregressive model.}
A per-subject generative model of $N_{\mathrm{p}}=121$ regions must be estimated
from a single session of $\sim$$140$ volumes, and that ratio, not a preference
for any particular architecture, dictates the choice. A subject-specific vector
autoregression would require $N_{\mathrm{p}}^{2}$ coefficients per lag: at one
lag that is $121$ predictors per region estimated from $139$ samples, already at
the edge of identifiability, and the temporal structure of interest spans
several lags, where the problem is plainly underdetermined. The reservoir moves
that burden off the data. Its recurrent and input operators are fixed and
\emph{identical for every session}, so the only quantity estimated per subject is
a ridge-regularised linear read-out on a common state basis, whose effective
degrees of freedom are set by the regularisation rather than by the raw parameter
count. The distribution of timescales that a vector autoregression would have to
buy with many lags is instead supplied by the reservoir's own dynamics.

Two further properties matter for what follows, and neither is available from a
fitted autoregression. Because $\mathbf{J}$ and $\mathbf{J}_{\mathrm{in}}$ are
shared, read-outs from different subjects are expressed in the same coordinates,
so differences between them ($\Delta\mathbf{W}$), group averages
($\bar{\mathbf{W}}_{\mathrm{CC}}$) and interpolations between individuals are
meaningful operations rather than comparisons of separately parameterised
systems. And the reservoir carries an explicit input pathway
$\mathbf{J}_{\mathrm{in}}$ that is distinct from its recurrent dynamics, so
injecting a drive at one region is well defined and separable from the model's
intrinsic behaviour. Both are prerequisites for the perturbation analyses
reported above.
...
(Fig.~\ref{fig:singlecompare}, figs.~S5,~S6); the two are therefore compared on a
...
\begin{figure}[htbp]
  ...
  patients unavoidably requires a large perturbation (distance $\gtrsim0.66$, cf.\ fig.~S5). Resonance is nonetheless the most \emph{efficient} axis, the
...
\begin{figure}[htbp]
  ...
  unperturbed one, and the mean $|FC|$ at those sites (Methods,
  ``Physiological diagnostics''); mean $\pm1\sigma$ over AD patients.
