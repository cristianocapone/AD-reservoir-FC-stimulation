...
even partial effects (fig.~S10). An alternative focal
...
(Fig.~\ref{fig:singlecompare}A; fig.~S7). Across both interpolation and oscillatory protocols,
...
reversions still require supra-physiological amplitudes (fig.~S10); their
...
few steps, from a sliding window of the model's own past output (Supplementary Methods). The
...
stimulation~\cite{spiegler2016,kunze2016}. Because the reservoir dynamics and the read-out are \emph{linear}, the optimal site and frequency follow directly from the system's transfer function and eigenspectrum rather than from an exhaustive search: the observed resonance is the peak of this linear frequency response, so the prescription is closed-form and analytically tractable. Notably, this optimum is
...
Reassuringly, the leading ``drive-toward-health'' site falls in the Default-network prefrontal cortex (fig.~S3), a hub consistently disrupted in AD~\cite{greicius2004,sheline2013} and a locus for non-invasive neuromodulation~\cite{koch2022}, lending biological plausibility. The heterogeneity of the per-patient optimal targets further argues
...
own past output rather than an offline oracle, reclassifies $\sim$$79\%$ of patients
(approaching the $\sim$$88\%$ open-loop rate) while cutting both the stimulation
dose ($\sim$$31\%$) and the departure from their own dynamics ($\sim$$5\%$ among
...
supra-physiological open-loop drive, is sought. These are \emph{model-level} prescriptions: the reservoir is fit to infraslow BOLD connectivity, not electrophysiology, so an ``intervention'' is an abstract input to regional FC dynamics, to be read as a target for regional drive rather than a cellular or oscillatory mechanism. Translating it to a specific technology, and validating it against electrophysiological or longitudinal outcomes, is left to future work.

More broadly, an identifiable individualised generative surrogate, fit once, serves simultaneously as a biomarker substrate and as a counterfactual testbed: the same fitted object that classifies the disease can be perturbed to ask what would reverse it, closing a loop from decoding to intervention that descriptive analyses cannot.
...
$\mathrm{RMS}_{\mathrm{pert}}/\mathrm{RMS}_{\mathrm{base}}>1$ (fig.~S10).
...
(Fig.~3A--D); all in-silico stimulation (Figs~4--6, S2--S10)
...
\begin{figure}[htbp]
  ...
  same as a disease-discriminative one} ($N=33$ AD). Because a resonant drive
  ...
  rate spans the whole low range ($6$--$45\%$, mean $19\%$; no site reaches
  ...
  discriminative (AUC $0.46$, rank $24/40$). (\textbf{C},\textbf{D}) At a fixed site
...
\begin{figure}[htbp]
  ...
  focal oscillatory stimulation} ($N=33$ AD). A sinusoidal drive
