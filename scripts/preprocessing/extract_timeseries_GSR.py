#!/usr/bin/env python3
"""
Extract parcel timeseries WITH Global Signal Regression (GSR).

Identical to extract_timeseries.py except:
  - Output: timeseries_GSR/  (original timeseries/ is untouched)
  - Confounds: adds global_signal + global_signal_derivative1
...
OUT_BASE  = Path(r"C:\Users\user\Desktop\2026.AD_MotionCorrection\timeseries_GSR")
...
PHYSIO_COLS     = ("csf", "white_matter",
                   ...
                   "global_signal", "global_signal_derivative1")   # <-- GSR added
...
def build_combined_atlas():
    ...
    ho_img = image.resample_to_img(ho_img_orig, sch_img, interpolation="nearest")

    ho_labels_all = [l.decode() if isinstance(l, bytes) else l for l in ho.labels]
    keep_idx = {i: name for i, name in enumerate(ho_labels_all) if i != 0}
    ...
    sub_labels = []
    new_label  = 100
    for ho_val, ho_name in sorted(keep_idx.items()):
        new_label += 1
...
def process_bold(bold_path: Path, atlas_img, masker_cache: dict, out_path: Path):
    ...
    ts     = masker_cache[masker_key].fit_transform(str(bold_path), confounds=conf)
    ts_out = ts.T   # (N_parcels, T)
...
def main():
    ...
    print(f"Labels saved ({len(all_labels)} parcels)")
    ...
    print("Building AD session -> patient mapping ...")
